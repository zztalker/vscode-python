...
//export async function getOrCreateInteractiveWindow(ioc: DataScienceIocContainer): Promise<IInteractiveWindow> {
    ...
    //// During testing the MainPanel sends the init message before our interactiveWindow is created.
...
export async function addCode(interactiveWindowProvider: () => Promise<IInteractiveWindow>, wrapper: ReactWrapper<any, Readonly<{}>, React.Component>, code: string, expectedRenderCount: number = 5): Promise<ReactWrapper<any, Readonly<{}>, React.Component>> {
    ...
    return getCellResults(wrapper, expectedRenderCount, async () => {
        const interactiveWindow = await interactiveWindowProvider();
        await interactiveWindow.addCode(code, 'foo.py', 2);
    });
}
